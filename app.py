...
class MediaDownloaderAPI:
    ...
    def _analyze_stream_link(self, url):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                uploader = info.get('uploader') or info.get('extractor_key') or urllib.parse.urlparse(url).netloc
    ...
    def _download_stream_yt(self, download_id, url, format_choice, output_dir, custom_title=None, custom_headers=None):
        ...
        # Assicura estensione .m3u8 per gli stream Vixcloud/HLS per evitare che yt-dlp li consideri pagine HTML
    ...
    def _notify_progress(self, download_id, percent, speed_bytes, downloaded_bytes, total_bytes):
        if not self._window:
            return
